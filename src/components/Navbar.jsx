...
const Navbar = ({ activePage, handleNavClick, isNavbarOpen, toggleNavbar }) => {
  return (
    <>
      <div className={`2xl:w-80 xl:w-52 w-44 h-full bg-[#f8f8f8] flex flex-col justify-between pt-5 pl-6 pb-14 md:pb-0 absolute md:relative z-10 transition-transform duration-300 -translate-x-110 ${isNavbarOpen ? 'translate-x-0' : '-translate-x-110 md:-translate-x-0'}`}>
        ...
        <div className="flex flex-col flex-grow">
          {navItems.map((item, index) => (
            <a 
              key={index}
              href="#" 
              className="xl:w-36 lg:w-32 flex items-center justify-between my-3.5 text-left cursor-pointer"
        ...
        <div className="flex justify-between pr-6 pb-2">
          <a href="#">
            <i className="bxl-instagram text-2xl text-red-600"></i>
          </a>
          <a href="#">
      ...
      <button className='fixed md:hidden bottom-4 left-4 text-4xl text-blue-400 z-30' onClick={toggleNavbar}>
